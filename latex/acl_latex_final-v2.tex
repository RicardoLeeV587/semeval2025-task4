...
%为了进一步明确数据增强的增益并非简单的训练集规模增大导致每个 epoch 的更新次数增加，我们设计了表4中的实验，我们分别对比了epoch为3，4，5时采用了数据增强的方案，与相同步数下不使用数据增强的方案，我们可以发现随着训练步数的增加虽然确实可以带来效果的增强，但是这些增益仍然是小于数据增强带来的增益。
